...
if ($script:edrBlindStatus.Count -gt 0) {
    Write-Host ""
    Write-Host "[EDR / XDR Agents]" -ForegroundColor Cyan
    Write-Host "  [INFO] The following audit subcategories are NOT compensated by EDR/XDR" -ForegroundColor DarkGray
    Write-Host "  [INFO] agent sensors (Defender for Endpoint, Cortex XDR, etc.)." -ForegroundColor DarkGray
    Write-Host "  [INFO] These events are only available through the Windows Security Log:" -ForegroundColor DarkGray
    ...
    if ($disabledCount -gt 0) {
        Write-Host "  [INFO] Fix disabled subcategories via 'WELA.ps1 configure' or GPO." -ForegroundColor DarkGray
    }
}
