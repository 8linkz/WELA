...
# EDR/XDR note (only shown when relevant audit subcategories failed)
if ($script:edrBlindFails.Count -gt 0) {
    Write-Host ""
    Write-Host "[EDR / XDR Agents]" -ForegroundColor Cyan
    Write-Host "  [INFO] If you are using an EDR/XDR (Defender for Endpoint, Cortex XDR, etc.)," -ForegroundColor DarkGray
    Write-Host "  [INFO] the following failed audit subcategories are NOT compensated by the" -ForegroundColor DarkGray
    Write-Host "  [INFO] agent's own sensors. These events are only available through the" -ForegroundColor DarkGray
    Write-Host "  [INFO] Windows Security Log:" -ForegroundColor DarkGray
    foreach ($name in $script:edrBlindFails) {
        Write-Host "  [INFO]   - $name" -ForegroundColor Yellow
    }
    Write-Host "  [INFO] Fix via 'WELA.ps1 configure' or GPO." -ForegroundColor DarkGray
}
